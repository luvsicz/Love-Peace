 ...
 * @author L
 * @version 1.0
 * @date 2019/4/18 15:28 Created by yuandl on 2016-12-16. 数据库JDBC连接工具类
 */
public class DBUtils {
  ...
  public static List<Map<String, Object>> query(String tableName,
    ...
    return query(tableName, false, null, whereClause, whereArgs, null, null,
            null, null);
  }

  /**
   * 执行sql通过 Map<String, Object>限定查询条件模糊查询 注意 这是多表
   *
   * @param sql        执行的语SQL句
   * @param whereMap   where条件
  ...
  public static List<Map<String, Object>> queryLikeMultLimit(String sql,
                                                             Map<String,
                                                                     Object> whereMap, Boolean startWith2) throws Exception {
  ...
  public static List<Map<String, Object>> queryLikeMultLimit(String sql,
                                                             Map<String,
                                                                     Object> whereMap, int cols) throws Exception {
    StringBuffer SQL = new StringBuffer();
    SQL.append(sql);
    if (whereMap != null && whereMap.size() > 0) {
      Iterator<String> iterator = whereMap.keySet().iterator();
  ...
  /**
   * 执行sql通过 Map<String, Object>限定查询条件模糊查询 注意 这是单表
   *
   * @param tableName 表名
   * @param whereMap  where条件
   * @return List<Map < String, Object>>
   ...
   *                 null
   *                 will return all rows for the given URL.
   ...
   *                 clause (excluding the HAVING itself). Passing null will
   *                 cause all row groups to be included, and is required when
   ...
   */
  private static void appendClause(StringBuilder s, String name,
                                   String clause) {
    if (!isEmpty(clause)) {
      s.append(name);
      s.append(clause);
