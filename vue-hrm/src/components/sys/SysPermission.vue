...
<script>
  export default {
    ...
    methods: {
      deleteRole(role) {
        this.$confirm('是否已经取消【' + role.nameZh + '】关联的菜单和用户?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.deleteRequest("/sys/permiss/role/" + role.id).then(resp => {
            if (resp) {
              this.initRoles();
            }
          })
      ...
      doAddRole() {
        if (this.role.name && this.role.nameZh) {
          ...
          this.postRequest("/sys/permiss/role", this.role).then(resp => {
      ...
      initSelectedMenus(rid) {
        this.getRequest("/sys/permiss/mids/" + rid).then(resp => {
          if (resp) {
      ...
      initAllMenus() {
        this.getRequest("/sys/permiss/menus").then(resp => {
          if (resp) {
            this.allmenus = resp;
          }
        })
      ...
      initRoles() {
        this.loading = true;
        this.getRequest("/sys/permiss/").then(resp => {
          this.loading = false;
          if (resp) {
            this.roles = resp;
          }
